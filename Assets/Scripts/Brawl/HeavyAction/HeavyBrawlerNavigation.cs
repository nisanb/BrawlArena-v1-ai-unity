...
{
    /// <summary>
    /// NavMesh planning boundary for an AI brawler. The bound child agent may
    /// calculate paths and desired velocity, but it never owns the actor
    /// Transform: position, rotation, and off-mesh traversal writes stay
    /// disabled while the heavy Rigidbody motor remains authoritative.
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class HeavyBrawlerNavigation : MonoBehaviour, IBrawlerNavigation
    {
        const float Acceleration = 40f;
        const float MinimumProbeRadius = 0.05f;
        // Knockback impulses and rolls are physically swept against world
        ...
        /// The component and agent remain disabled until assembly opens them.
        /// </summary>
        public void ConfigureDormant(NavMeshAgent configuredAgent)
        {
                ...
                !configuredAgent.transform.IsChildOf(transform))
            {
                throw new ArgumentException(
                    "The planning agent must live on one dedicated child.",
            ...
            if (agents.Length != 1 || agents[0] != configuredAgent)
            {
                throw new InvalidOperationException(
                    "The AI topology requires exactly one child NavMeshAgent.");
            }
        ...
        public void Initialize(float moveSpeed, float stoppingDistance)
        {
            ...
            if (initialized)
            {
                ...
                {
                    throw new InvalidOperationException(
                        "The initialized navigator cannot adopt different movement settings.");
                }
                return;
            }
        ...
        public void ResetRuntimeTrace()
        {
            ...
            {
                throw new InvalidOperationException(
                    "Close the AI planner before resetting runtime trace.");
            }
            ResetTraceFields();
        }
        ...
        void EnsureConfigured()
        {
            ...
            {
                throw new InvalidOperationException(
                    "Configure the navigator with exactly one dedicated child agent first.");
            }
        }
